...
describe('redux pagination reducer factory', function() {
  describe('pagination reducer configuration', function() {
    it('stores its state at the configured key', function() {
      ...
      expect(state.pagination.myKey).to.exist;
      expect(state.pagination.pagination).to.not.exist; // default

      expect(keySelectors.selectPaginationOrder(state)).to.eql({ 0: 1 });
    });
    ...
    it('selects entities with a custom idAttribute', function() {
      ...
      expect(idAttributeSelectors.selectPaginationOrder(state)).to.eql({
        0: 1
      });
    });
    ...
    it('selects the response result using the configured function', function() {
      ...
      expect(resultSelectors.selectPaginationOrder(state)).to.eql({ 0: 1 });
    });
    ...
    it('selects the response total using the configured function', function() {
      ...
      expect(totalSelectors.selectTotalEntities(state)).to.equal(100);
    });
    ...
    it('creates a paginator for each unique key when given a keyBy string', function() {
      ...
      expect(id1Selectors.selectPaginationOrder(state)).to.eql({ 0: 0 });
      expect(id1Selectors.selectTotalEntities(state)).to.equal(1);

      expect(id2Selectors.selectPaginationOrder(state)).to.eql({ 0: 0, 1: 1 });
      expect(id2Selectors.selectTotalEntities(state)).to.equal(2);
    });
  ...
  describe('pagination reducer', function() {
    it('updates the status when a call is made', function() {
      ...
      expect(testSelectors.selectFetchingStatus(state)).to.equal(
    ...
    it('ignores requests that have been superseded', function() {
      ...
      expect(testSelectors.selectFetchingStatus(finishedState)).to.equal(
        fetchingStatus.success
      );
    });
    ...
    it('stores/retrieves engines at the correct paginated order', function() {
      ...
      expect(testSelectors.selectPaginationOrder(state)).to.eql({
        0: 'id-0',
        1: 'id-1',
        2: 'id-2',
        3: 'id-3',
    ...
    it('properly stores/retrieves totals', function() {
      ...
      expect(testSelectors.selectTotalEntities(state)).to.equal(100);
    });
    ...
    it('filters actions so the same constants can be used for multiple paginators', function() {
      ...
      expect(filterSelectors.selectPaginationOrder(state)).to.eql({ 0: 1 });
    });
    ...
    it('stores/retrieves indices of fetching entities', function() {
      ...
      expect(testSelectors.selectFetchingEntities(state)).to.eql([0, 1]);
      ...
      expect(testSelectors.selectFetchingEntities(successState)).to.eql(
        [],
    ...
    it('stores/retrieves indices of failed entity fetches', function() {
      ...
      expect(testSelectors.selectFailedFetchingEntities(state)).to.eql([0, 1]);
      ...
      expect(testSelectors.selectFailedFetchingEntities(successState)).to.eql(
        [],
        'clears after success'
      );
    });
    ...
    it(
      ...
      function() {
        ...
        expect(clearSelectors.selectPaginationOrder(state)).to.eql({});

        expect(clearSelectors.selectTotalEntities(state)).to.eql(2);
      }
    );
    ...
    it(
      ...
      function() {
        ...
        expect(invalidateSelectors.selectPaginationOrder(state)).to.eql({});

        expect(invalidateSelectors.selectTotalEntities(state)).to.eql(2);
      }
    );
  });
});
